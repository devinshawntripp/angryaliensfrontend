...
export default function NewHome() {
    ...
    const mintNFTHandler = async () => {
		...
		if (app.angryAliens && app.address) {
			...
			await app.angryAliens.methods.mint(1).send({ from: app.address, value: 0 })
				.on('confirmation', async () => {
					const maxSupply = await app.angryAliens.methods.maxSupply().call()
					const totalSupply = await app.angryAliens.methods.totalSupply().call()
                    ...
                    dispatch(loadAppData({...app, supplyAvailable: maxSupply-totalSupply, tokensOwned: ownerOf }))
				})
				.on('error', (error) => {
					window.alert(error)
					// setIsError(true)
            ...
            <main>
                ...
                <section id='about' className='about'>
                        ...
                        <Row className='flex m-3'>
                            ...
                            <Col md={5} lg={4} xl={5} xxl={4} className='text-center'>
                                <img src={showcase} alt="Multiple Angry Aliens" className='showcase' />
                            </Col>
                            <Col md={5} lg={4} xl={5} xxl={4}>
                                {app.isError ? (
                                    <p>{app.message}</p>
